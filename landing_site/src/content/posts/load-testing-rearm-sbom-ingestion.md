...
The Dependency-Track 5.0 announcement mentions early adopters ingesting "upwards of 20,000 SBOMs per hour" - with no published SBOM size, hardware spec, or definition of "ingested". We decided to find out what those numbers look like for ReARM when every one of those variables is stated - and to publish the harness so anyone can re-run it.
...
- **Unit of work: a release carrying one SBOM.** A release is what ReARM models; driving bare SBOMs onto a handful of releases would measure something nobody operates. Each run creates 20,016 releases, each with a 50-component CycloneDX SBOM (~24 KB) synthesized from a pool of 2,867 real components extracted from a production ReARM build - real purls, unique content per release, no dedup shortcuts.
- **Two completion markers, reported separately.** *Accepted* means the API returned and the release is queryable. *Fully processed* means the SBOM's components are reconciled and queryable per release - our honest equivalent of "ingested". A number without this distinction is trivially inflatable, because upload APIs (ours and Dependency-Track's alike) return before analysis completes.
...
### The honest hardware disclosure
...
The processing (reconciliation) throughput ceiling turned out to be a scheduler batch-size constant. It is now a configurable property (`SBOM_RECONCILE_DRAIN_BATCH`) - at its tuned setting the post-burst backlog drains at ~250 releases/minute instead of ~66, taking the 20k end-to-end time on an in-cluster registry from over 6 hours to 1h21m. That change, along with repository-name validation and client caching in the OCI artifact service, is merging alongside this post.

One scope note: the instance's Dependency-Track integration remained enabled throughout the campaign, but BEAR was not in play - these numbers measure ReARM's own ingestion and reconciliation pipeline.

### Reproduce it

The harness - component pool builder, JMeter plan, completion probes and every run record referenced above - is in the ReARM repository under `perf/sbom-ingest`. The README states every ceiling, batch size and dedup layer that shapes the numbers, so if you get different results, you will be able to say exactly why.
