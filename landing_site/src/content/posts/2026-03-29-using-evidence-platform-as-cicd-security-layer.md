...
Following the recent Trivy and LiteLLM supply chain attacks, it is a common view that we don't have a good way to protect our CI layer, specifically GitHub Actions, from supply chain attacks.
...
Evidence platform such as [ReARM](https://rearmhq.com) can serve as a security layer for CI/CD workflows. Evidence platform by definition creates a verifiable record of supply chain events and artifacts that are stored outside of the actual CI/CD system. Of course, an evidence platform may be subject to attack as well, but in case of compromise an attacker would need to breach both the CI/CD system and the evidence platform. Thus, such attack would be significantly more difficult to carry out. In other words, an evidence platform may be used as similar to a 2FA - it adds an additional layer of security to the CI/CD workflow.
...
1. CI stage that builds initial (non-release) artifacts may still belong to the main repository
2. Evidence platform that verifies the artifacts and stores them
3. Approval and/or release lifecycle rules within the evidence platform to gate the release
...
Note that for ReARM specifically, approvals and triggering workflows is only a ReARM Pro functionality, i.e. as [described here for GitHub Actions](https://docs.rearmhq.com/integrations/githubActionsTriggers.html). However, even ReARM CE users can leverage GitHub Actions scheduler and use ReARM CLI to [request latest release with desired lifecycle](https://github.com/relizaio/rearm-cli?tab=readme-ov-file#4-use-case-request-latest-release-per-component-or-product).
