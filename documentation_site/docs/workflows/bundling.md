...
- **Never Append** - never append a Feature Set suffix. On a version conflict with a sibling Feature Set, the next version continues the component-wide sequence (e.g., `0.1.0`, `0.1.1` → next is `0.1.2`); `-0`, `-1`, `-2` suffixes are only used when no new version can be derived (e.g., a pinned follow version).
