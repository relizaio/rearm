...
describe('the FDA addendum export is wired into the export modal', () => {
    ...
    it('declares the bom-type watch below the ref it watches', () => {
        // Matched WITHOUT pinning the type annotation. The previous version hardcoded
        // `Ref<string>` and failed the moment the ref was correctly narrowed to the
        // ExportBomType union -- a spec that guards declaration ORDER should not also freeze
        // the declaration's type, or every improvement to it reads as a regression.
        const decl = source.search(/const exportBomType(: Ref<[^>]+>)? = ref\('SBOM'\)/)
