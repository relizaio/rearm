...
const baseVariableDocs: VariableDoc[] = [
    ...
    { name: 'release.dependencies[].lifecycle',  snippet: 'release.dependencies.all(d, d.lifecycle == "READY_TO_SHIP")',     display: 'release.dependencies[].lifecycle',  desc: 'string — the dependency\'s lifecycle by name (PENDING, DRAFT, ASSEMBLED, READY_TO_SHIP, GENERAL_AVAILABILITY = Shipped, END_OF_*). Equality is exact: use it where one state is the rule, and the maturity rank where "at least this far" is.' },
    { name: 'release.dependencies[].maturity',   snippet: 'release.dependencies.all(d, d.maturity >= 3)',                    display: 'release.dependencies[].maturity',   desc: 'int — a rank over the dependency\'s LIFECYCLE, so a rule can say "at least this far" without listing lifecycles by hand: -1 Rejected/Cancelled, 0 Pending, 1 Draft, 2 Assembled, 3 Ready to Ship, 4 Shipped and every retirement lifecycle after it. Answers "how far has it got" and nothing else — a dependency at End of Life still scores 4, because it did ship.' },
    { name: 'release.dependencies[].supported',  snippet: 'release.dependencies.all(d, d.supported)',                        display: 'release.dependencies[].supported',  desc: 'bool — still something to build on. False at End of Support, End of Life, Rejected and Cancelled. The question the maturity rank cannot answer, so "Shipped and still supported" asks for both: d.maturity &gt;= 4 &amp;&amp; d.supported.' },
    ...
    { name: 'release.agentSessions[].hasAwaitingPolicy', snippet: 'release.agentSessions.exists(s, s.hasAwaitingPolicy)', display: 'release.agentSessions[].hasAwaitingPolicy', desc: 'bool — any session policy still AWAITING' },
    ...
    { name: 'release.agentSessions[].awaitingPolicies',  snippet: 'release.agentSessions.exists(s, s.awaitingPolicies.size() > 0)', display: 'release.agentSessions[].awaitingPolicies', desc: 'list&lt;string&gt; — policy UUIDs currently in AWAITING state' },
