...
const baseVariableDocs: VariableDoc[] = [
    ...
    { name: 'release.dependencies[].maturity',   snippet: 'release.dependencies.all(d, d.maturity >= 3)',                    display: 'release.dependencies[].maturity',   desc: 'int — ordering of that lifecycle: -1 cancelled/rejected, 0 pending, 1 draft, 2 assembled, 3 ready to ship, 4 general availability and beyond. Baselined is &gt;= 3, shipped is &gt;= 4. Answers "how far has it got", nothing else — an end-of-life dependency still scores 4.' },
    { name: 'release.dependencies[].supported',  snippet: 'release.dependencies.all(d, d.supported)',                        display: 'release.dependencies[].supported',  desc: 'bool — still something to build on. False for end-of-support, end-of-life, cancelled and rejected. The question maturity cannot answer, so a shipped-and-supported rule asks for both.' },
