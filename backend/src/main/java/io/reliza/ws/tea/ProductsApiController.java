...
public class ProductsApiController implements ProductsApi {
    ...
    @Override
    public ResponseEntity<TeaInlineObject> getTeaProductByIdentifier(
            ...
            @Parameter(name = "idType", description = "Type of identifier specified in the `idValue` parameter", in = ParameterIn.QUERY) @Valid @RequestParam(value = "idType", required = false) TeaIdentifierType idType,
            @Parameter(name = "idValue", description = "If present, only the objects with the given identifier value will be returned.", in = ParameterIn.QUERY) @Valid @RequestParam(value = "idValue", required = false) String idValue
        ) {
    	...
    	TeaInlineObject tio = new TeaInlineObject();
    	tio.setPageSize(Long.valueOf(100));
    	tio.setPageStartIndex(Long.valueOf(0));
    	tio.setTotalResults(Long.valueOf(teaProducts.size()));
    	tio.setResults(teaProducts);
    	tio.setTimestamp(OffsetDateTime.now(ZoneOffset.UTC).truncatedTo(ChronoUnit.SECONDS));
    	return ResponseEntity.ok(tio);
    }
