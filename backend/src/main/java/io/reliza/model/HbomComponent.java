...
/**
 * A parsed HBOM (hardware BOM) component for a release — one device/firmware
 * node extracted from the release's hardware BOM artifact. Per-release cache,
 * rebuilt by {@code HbomComponentService}; unlike SbomComponent it is not
 * canonically deduplicated across the org (hardware nodes have no purl).
 */
@Entity
@Table(schema = ModelProperties.DB_SCHEMA, name = "hbom_components")
public class HbomComponent implements Serializable, RelizaEntity {
	private static final long serialVersionUID = 234770;
