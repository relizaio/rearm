...
public enum DeviceSupportRisk {
	...
	/**
	 * Triage order: the actionable verdict first, then what could not be assessed, then OK.
	 * Owned here for the same reason {@link #isFlagged()} is -- every surface that sorts by
	 * verdict uses this one rank so a new verdict cannot silently sort last on one screen and
	 * first on another. Declaration order above is NOT this order and must not be relied on.
	 */
	public int triageRank() {
		return switch (this) {
			case EOS_BEFORE_DEVICE -> 0;
			case UNKNOWN -> 1;
			case OK -> 2;
		};
	}
